import type { AyahMeta, JuzMeta, SurahMeta } from "./types"
import { AYAH_PAGE_METADATA } from "./page-coverage-metadata"
import { JUZ_RANGES } from "./juz-ranges"
...
export const AYAH_METADATA: AyahMeta[] = AYAH_PAGE_METADATA

export const JUZ: JuzMeta[] = JUZ_RANGES.map((juz) => ({
  id: juz.id,
  name: `الجزء ${juz.id}`,
  surahIds: [...new Set(juz.ranges.map((range) => range.surahId))],
  ranges: juz.ranges,
}))
...
export function getJuzMeta(juzId: number): JuzMeta | undefined {
  return JUZ.find((juz) => juz.id === juzId)
}
